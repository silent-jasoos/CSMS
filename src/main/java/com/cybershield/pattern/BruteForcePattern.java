...
public class BruteForcePattern implements AttackPattern {
    ...
    public boolean detect(Map<String, Object> eventData) {
        ...
        if (failures.size() >= FAILURE_THRESHOLD_COUNT) {
            ...
            alert.setDetectedAt(LocalDateTime.now());  // ✅ FIXED: Use setDetectedAt() not setTimestamp()
            alert.setResolved(false);
            alertRepository.save(alert);

            attackLogger.log("IDS_ALERT", ip, "HIGH", null,
                    "Brute force attack detected: " + failures.size() + " failures");
